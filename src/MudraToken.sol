...
contract MudraToken is ERC20, ERC20Burnable, Ownable, ReentrancyGuard, Pausable {
    ...
    error CannotWhitelistBlacklistedAddress();
    ...
    /**
     * @dev Override ownership transfer to revoke roles from previous owner
     */
    function _transferOwnership(address newOwner) internal override {
        ...
        if (oldOwner != address(0)) {
            isWhitelister[oldOwner] = false;
            isBlacklister[oldOwner] = false;
            isWhitelisted[oldOwner] = false;

            emit WhitelisterStatusUpdated(oldOwner, false);
            emit BlacklisterStatusUpdated(oldOwner, false);
            emit AddressWhitelisted(oldOwner, false);
        }

        // Grant roles to new owner
        isWhitelister[newOwner] = true;
        isBlacklister[newOwner] = true;
    ...
    function setWhitelisted(address account, bool status) external onlyWhitelister {
        ...
        // Prevent whitelisting blacklisted addresses (audit recommendation)
        if (status && isBlacklisted[account]) {
            revert CannotWhitelistBlacklistedAddress();
        }
    ...
    /**
     * @dev Set blacklisted status for an address
     * @param account Address to update
    ...
    /**
     * @dev Batch set blacklisted status for multiple addresses
    ...
    function batchWhitelist(address[] calldata accounts, bool status) external onlyWhitelister {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            // Prevent whitelisting blacklisted addresses (audit recommendation)
            if (status && isBlacklisted[accounts[i]]) {
                continue; // Skip blacklisted addresses in batch operations
            }
    ...
    /**
     * @dev Override _update to allow burning from blacklisted addresses
     */
    function _update(address from, address to, uint256 amount) internal virtual override whenNotPaused {
        // Check for blacklisted addresses
        if (from != address(0)) {
            // Allow burning from blacklisted addresses (to == address(0) means burn)
            if (to != address(0)) {
    ...
    /**
     * @dev Override _spendAllowance to check if spender is blacklisted
     */
